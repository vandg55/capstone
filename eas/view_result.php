...
<div class="container-fluid" id="printable">
	...
	<table class="table table-bordered">
		...
		<tbody>
			<?php 
    			$items=$conn->query("SELECT r.*,s.subject_code,s.subject,s.Lec,s.Lab,s.Prereq FROM result_items r inner join subjects s on s.id = r.subject_id where result_id = $id  order by s.subject_code asc");
    			while($row = $items->fetch_assoc()):
    		?>
    		<tr>
    			<td><?php echo $row['subject_code'] ?></td>
    			<td><?php echo ucwords($row['subject']) ?></td>
				<td><?php echo ucwords($row['Lec']) ?></td>
				<td><?php echo ucwords($row['Lab']) ?></td>
				<td><?php echo ucwords($row['Prereq']) ?></td>
